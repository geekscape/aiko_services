...
locations, such as files, a terminal, a socket or a camera stream. A
DataTarget *stores* frames at given locations. The locations are URLs
supplied through the `data_sources` and `data_targets`
[Parameters](parameters.md). The URL scheme selects a pluggable
[DataScheme](scheme.md) implementation, which does the I/O. Thus one
element class works across many transports.

Concrete subclasses live with the media elements, for example `TextReadFile`,
`ImageReadFile`, `VideoReadFile`, `AudioWriteFile`, `TextWriteZMQ`
(`src/aiko_services/elements/media/`) and the GStreamer RTSP elements.

**Why to use it**: write the domain logic (`process_frame()`) once and
...
DataSource and DataTarget have no CLI of their own. `aiko_pipeline`
exercises them. Use `-p` (Pipeline parameters) or `-sp` (Stream
parameters) to set their URLs and controls. These examples come from the
...
|-----------|-----------|---------|
| `data_sources` | DataSource | List of URLs to load data from (**needed**) |
| `data_targets` | DataTarget | List of URLs to store data at (**needed**) |
...
The URL list is an S-expression, for example
...
with a frame generator (which needs a thread). Subclasses may also
...
variables the scheme prepared (for example, `target_path`,
`target_path_template`, `target_file_id` for the `file` scheme).

There is no wire protocol specific to these classes — frames travel through
...
- `use_create_frame=True` is only a *permission* — the scheme decides
  (for example, `DataSchemeFile` uses `create_frame()` only when exactly one
  path resulted).
...
- Design refactor to disambiguate DataSource and DataScheme
  implementations — in particular, generalize
