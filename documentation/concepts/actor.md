...
examples are Actors. A plain Service gives you discovery and message
topics. To that, Actor adds these items:

- The mailbox discipline
- Built-in [shared state](share.md) (`self.share` with an ECProducer)
- A per-Actor logger with a remotely adjustable log level
- Message [Hooks](hook.md) for instrumentation

**Why to use it**: any component that owns mutable state and must be
...
Note that even the local `actor_test.test()` call above runs through the
...
Actor has no command-line tool of its own. Every Actor-based tool
exercises it (`aiko_dashboard`, `aiko_pipeline`, `aiko_registrar`,
`aiko_hyperspace`). The hello-world examples in
`src/aiko_services/examples/aloha_honua/` show it most directly:
...
| `set_log_level(level)` | Update `share["log_level"]` through the ECProducer, which adjusts the Actor's logger — works remotely |
| `is_running()` | Return `share["running"]` |
| `_post_message(topic, command, args, delay=None, target_function=None)` | Post a `Message` to the Actor's own mailbox (protected; used by subclasses, for example, to defer work or schedule delayed commands) |
...
message and posts it to the mailbox after the delay through an event-loop
...
path. `ACTOR_HOOK_MESSAGE_IN` (`actor.message_in:0`) fires when a
message is posted. `ACTOR_HOOK_MESSAGE_CALL` (`actor.message_call:0`)
fires when the mailbox handler is about to invoke it.
...
- **Uniform local/remote calls.** The same mailbox path serves MQTT
  ...
  (`proxy_post_message`), so behavior does not depend on where the
  caller lives.
...
- **`Message`** captures `(target_object, command, arguments,
  ...
  notes that the `except Exception` was `except TypeError` only, and
  that a catch of everything hides bugs in the target function.
...
- **Delayed delivery is approximate.** `_post_message(delay=...)` puts
  ...
  queue to the mailboxes, whatever each entry's deadline is. Thus later
  entries with longer delays are delivered early. Entries added while a
  longer timer is pending must wait for it. Treat `delay` as
...
| `ActorImpl` | Create mailboxes and subscribe `topic_in`; wrap commands as Messages and post them (`_post_message`, `proxy_post_message`); drain through `_mailbox_handler`; manage `share`/ECProducer, logger and delayed messages; run the event loop | [Event](event.md) (mailboxes, timers), `ECProducer` in [Share](share.md), [Hooks](hook.md), `aiko.process` |
...
- A lightweight `ActorCoreImpl` if the default `ActorImpl` proves too
  heavy
- A true **priority mailbox for internal needs**, for example, posting
...
- Rename "function" and "method" consistently. Consider a `uuid` suffix on
