...
| How do I *group and organize* those references? | [Category](category.md) | A directory |
...
generalized into a **distributed, directed network graph of live Service
references**: a mash-up of hypermedia ("hyperspace"), directed graphs and
file-system concepts. A file-system organizes inert files on one host.
HyperSpace instead organizes *references to running (or runnable) Services across
...
Because a Category **is-a** Dependency, any Category can be an Entry
inside another Category. That single rule produces arbitrarily deep,
navigable structures, with path names like
`category_a/category_b/entry_c`. StorageFile represents Entries as
*symbolic links* to content-addressed storage. Thus the same Entry can be
linked at several paths, which turns the tree into a directed graph.
...
1. An application developer **adds an Entry** to HyperSpace at a path, for example
   `agents/llm_gemma`, giving a *ServiceFilter* (and optionally a
...
2. When the Dependency needs to be *resolved*, the ServiceFilter is used for
   **discovery** through the Registrar. If the Service is already running, the
   Dependency's `service` field becomes a local or remote reference to it.
3. If the Service is *not* running, the `lifecycle_manager_url`
   identifies the LifeCycleManager that can **make the operating system
...
exposes its state through eventually-consistent shared state
