...
`lease_time` and needs periodic extension — silence means release.
...
- **[Share](share.md)**: an `ECProducer` holds an `ECLease` per remote
  ...
  extending. An `ECConsumer` holds one auto-extending Lease that re-sends
...
**Why to use it**: track claims that must vanish when their owner does
...
| Operation | Effect |
|-----------|--------|
| `Lease(lease_time, lease_uuid, lease_expired_handler=None, lease_extend_handler=None, automatic_extend=False)` | Start the expiry timer immediately; with `automatic_extend`, also start an extension timer at 0.8 × `lease_time` |
...
- **Grantor** (for example, `ECProducer`): creates a plain Lease per client with a
  `lease_expired_handler` that releases the resource. Calls `extend()`
  whenever the client extends. Calls `terminate()` when the client
  explicitly releases (lease time `0` in the share protocol).
- **Holder** (for example, `ECConsumer`): creates one Lease with
  `automatic_extend=True` and a `lease_extend_handler` that transmits the
  extension request — the Lease becomes a self-driving heartbeat.
...
- **A Lease is two [Event](event.md) timers** and nothing more — no
  ...
  removes itself. Extension, when enabled, is periodic at
  `_LEASE_EXTEND_TIME_FACTOR = 0.8` of the lease time. That leaves a 20 %
  margin for the extension message to arrive before the far side expires.
...
- With `automatic_extend`, the extension timer's period is fixed at
  construction (0.8 × the *original* `lease_time`). A later
  `extend(new_lease_time)` changes the expiry duration but not the extension
  cadence.
- `extend` doubles as the extension timer handler — timer handlers are
...
|-------|------------------|---------------|
| `Lease` | Hold `lease_time` / `lease_uuid`; run the expiry timer; optionally self-extend at 0.8 × lease time; notify expiry and extension handlers; cancel cleanly on `terminate()` | [Event](event.md) (timer handlers); handler callables supplied by the owner |
...
- Lease request *rejection* responses for grantors, for example
...
- The extension cadence does not track lease-time changes made through
  `extend(new_lease_time)` when `automatic_extend` is in use
- There is no unit test for Lease (nothing under
  `src/aiko_services/tests/` covers it). Timer-based behavior would suit
