...
`CategoryImpl.__init__()` initializes both parents via
`context.call_init(self, "Actor", ...)` and
...
### Entries live in shared state

Entries are not a private dict — they are stored in `self.share["entries"]`
and every mutation goes through the `ECProducer`:
...
### `update()` and the `0:` sentinel

`update()` merges field-by-field: only truthy fields of the incoming
ServiceFilter (and non-null URLs) are applied. Because "no change" must be
expressible over the wire, the CLI uses `0:` as the *None sentinel* for
name/protocol/transport/owner (S-expression-safe). `update()` also guards
against `ServiceFilter.__init__` substituting the local hostname for a null
name — it remembers whether the name was null and restores it.

### The `list` wire protocol

`list()` publishes to the requester's response topic:
...
Entry records come in two shapes (assembled in `_get_entry_records()` and
interpreted by `_list_print()`):
...
class-method utilities reused by the HyperSpace and StorageFile CLIs, so all
three present identical output:
...
All CLI verbs use the standard Aiko Services idiom — discover, invoke,
terminate:
