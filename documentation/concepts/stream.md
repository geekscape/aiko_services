...
It is the unit of session state that groups related Frames under a
`stream_id`. Such a group is a video, a sensor feed, or one batch job.
Each Stream has its own [parameters](parameters.md), response routing
and [lease](lease.md)-based expiry. Each Frame carries the accumulated data
...
**Why to use it**: to process many independent inputs through one
...
|-------------|-------|---------|
| `ERROR` | -2 | Do not generate new frames; ignore queued frames |
| `STOP` | -1 | Do not generate new frames; process queued frames |
...
makes the Stream first (default mode). Thus simple one-shot invocations
do not need an explicit `create_stream`.
...
- **Locking.** Every Stream owns a named `Lock` (created in
  ...
  `frame_generator()` call — this serializes generators against frame
...
- **`Stream.set_state()`** intends to prevent downgrades out of `ERROR`
  and `STOP`. But its `if / if-else` structure means that any state
  other than `STOP` falls into the final `else`, and is assigned
  unconditionally. Thus `RUN` can overwrite `ERROR`. This is a suspected
  bug. Treat the guards as intent, not as implemented behavior.
...
- The module docstring warns: do not confuse `stream.py:Frame` with
...
|-------|------------------|---------------|
| `Stream` (dataclass) | Identify the session (`stream_id`, `graph_path`); hold parameters, state, in-flight `frames`, response routing, `variables` and the per-Stream `Lock`; serialize (`as_dict()`) and re-hydrate (`update()`) for the wire | `Frame`, `Lock` utility |
