...
A device reboots when its handle closes. It leaves discovery within
half a second and returns 8 to 13 seconds later in the bootloader state.
A connection try in that window can crash the process inside the
SDK. So `open()` connects only to a device that discovery reports as
ready, and waits up to 30 seconds for it. An address that discovery
