...
description: Shared helper module for the GStreamer elements — lazy
  GStreamer/PyGObject initialization, platform H.264 codec selection,
...
`aiko_services.elements.gstreamer` package. It is not a
[PipelineElement](../../concepts/pipeline_element.md). It is the helper
module that two groups depend on: the current-style RTSP modules
([rtsp_io](rtsp_io.md), [scheme_rtsp](scheme_rtsp.md)), and the legacy
video wrappers ([video_reader](video_reader.md) and its family).
It owns three concerns:

1. **GStreamer initialization** — `gst_initialise()` does the lazy
   `gi.require_version` / `Gst.init([])` dance exactly once, optionally
   also loading `GstRtspServer` and `GLib` for RTSP serving.
2. **Platform codec selection** — Linux compared with Mac OS X H.264
...
**Why to use it**: any new GStreamer-based element starts here —
...
Needs either `pip install PyGObject` or
...
Exported through `__all__` (and re-exported by the package `__init__.py`):

| Function | Effect |
|----------|--------|
| `gst_initialise(multiple_return_values=False, rtsp_server=False)` | Idempotently initialize GStreamer; returns `Gst`, or the 5-tuple `(Gst, GstBase, GLib, GstRtspServer, GObject)` when `multiple_return_values` or `rtsp_server` is set |
...
| `process_video(video_reader, video_writer)` | Blocking copy loop: read frames, optionally display and overlay the frame id through OpenCV (`q` to quit), write frames |
...
- `gst_initialise()` caches the `gi.repository` modules in
  module-level globals, so every caller shares one initialized
...
- **Global-module pattern.** GStreamer bindings and OpenCV are held as
  module globals, and not passed around. This is simple. But it makes
  the import order and the `enable_opencv()` timing implicit
  contracts.
- **Platform table, not detection.** Codec choice is a two-entry dict
  ...
  GStreamer side.
- The OpenCV codec-probing trio belongs conceptually to the OpenCV
  writers of the media elements, and not to GStreamer.
  (`can_use_codec()` writes a throwaway `__test*` file to test a
  `VideoWriter`.) This shows that the module is taking on a second
  responsibility.
...
- `process_video()` assumes every queued frame has an `"image"` key
  ...
  writer correctly. The loop also never returns normally, except through
  the OpenCV `q` key.
...
| *(module functions)* | Initialize GStreamer once; supply platform codec/format names; opt-in OpenCV; run the legacy reader→writer copy loop; probe OpenCV codecs (WIP) | `gi` / `Gst` / `GstRtspServer`, `cv2`, every sibling module in this package |
...
- `operating_system` other than Linux / Mac OS X (for example, Windows) makes
