...
[DataScheme](../../concepts/scheme.md) plug-in design. It is the default
scheme, because a URL with no `scheme:` prefix is treated as `file`. It
is also the workhorse behind every file-based
...
It resolves `data_sources` URLs into an ordered list of `Path`s. Those
URLs name single files, whole directories, or `{}` glob templates. It
then chooses between one-shot `create_frame()` and a paced
frame-generator thread. It batches paths per frame, and prepares
template state for `data_targets`.

**Why to use it**: you do not invoke it directly — you select it with
a URL:
...
- A path (after stripping a `{}` template basename) that **is a
  ...
  `file_id` — the text the `*` matched (for example, `00` from `in_00.txt`).
...
- **`file_id` recovery.** `_file_glob_difference(file_glob, filename)`
  extracts the substring that the `*` matched. It does this when it
  strips the prefix and suffix of the glob. Thus a target template can
  number its outputs to agree with the source files.
- All mutable state (`source_paths_generator`, `target_*`) is stored in
  `stream.variables`, honoring the one-scheme-instance-per-
...
- In `create_sources()` the directory branch reuses the name `path` as
  its loop variable. The later single-path
  `create_frame(stream, {"paths": [path]})` call then relies on that
  variable to hold the intended path. This is correct for the common
  cases, but it is fragile. Prefer `paths[0][0]` in a refactor.
...
- Inherited from the [DataScheme](../../concepts/scheme.md) roadmap:
  replace the hand-rolled URL parsing with `urllib.parse.urlparse()`.
  `parse_url_path()` currently returns the authority and the path
  combined.
